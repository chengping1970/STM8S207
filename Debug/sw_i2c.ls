  ...
  25  000a 00            	dc.b	0
  63                     ; 59 static void _Delay_5us(void)
  63                     ; 60 {
  64                     	switch	.text
  65  0000               L71f__Delay_5us:
  67  0000 88            	push	a
  68       00000001      OFST:	set	1
  71                     ; 62    	for (i = 0;i < 10;i++);
  ...
  87  000b 84            	pop	a
  88  000c 87            	retf
 113                     ; 65 static void _SWI2C_Start(void)
 113                     ; 66 {
 114                     	switch	.text
 115  000d               L15f__SWI2C_Start:
 119                     ; 67 	GPIO_WriteHigh(IIC_SCL_PORT,IIC_SCL_PIN);
 121  000d 4b10          	push	#16
 122  000f ae5005        	ldw	x,#20485
 123  0012 8d000000      	callf	f_GPIO_WriteHigh
 125  0016 84            	pop	a
 126                     ; 68 	GPIO_WriteHigh(IIC_SDA_PORT,IIC_SDA_PIN);
 ...
 137                     ; 70 	GPIO_WriteLow(IIC_SDA_PORT,IIC_SDA_PIN);
 ...
 148                     ; 72 	GPIO_WriteLow(IIC_SCL_PORT,IIC_SCL_PIN);
 ...
 155                     ; 73 }
 158  003d 87            	retf
 183                     ; 75 static void _SWI2C_Stop(void)
 183                     ; 76 {
 184                     	switch	.text
 185  003e               L36f__SWI2C_Stop:
 189                     ; 77 	GPIO_WriteLow(IIC_SDA_PORT,IIC_SDA_PIN);
 ...
 200                     ; 79 	GPIO_WriteHigh(IIC_SCL_PORT,IIC_SCL_PIN);
 ...
 211                     ; 81 	GPIO_WriteHigh(IIC_SDA_PORT,IIC_SDA_PIN);
 ...
 218                     ; 82 }
 221  0064 87            	retf
 264                     ; 84 static u8 _SWI2C_SendByte(u8 value)
 264                     ; 85 {
 265                     	switch	.text
 ...
 291  007b               L321:
 292                     ; 96 			GPIO_WriteLow(IIC_SDA_PORT,IIC_SDA_PIN);
 ...
 304                     ; 99 		GPIO_WriteHigh(IIC_SCL_PORT,IIC_SCL_PIN);
 ...
 352                     ; 107 	GPIO_WriteHigh(IIC_SCL_PORT,IIC_SCL_PIN);
 354  00c5 4b10          	push	#16
 355  00c7 ae5005        	ldw	x,#20485
 356  00ca 8d000000      	callf	f_GPIO_WriteHigh
 358  00ce 84            	pop	a
 359                     ; 108 	for (count = 0;count < IIC_ACK_TIMEOUT;count++)
 361  00cf 0f01          	clr	(OFST+0,sp)
 362  00d1               L721:
 363                     ; 110 		if (GPIO_ReadInputPin(IIC_SDA_PORT, IIC_SDA_PIN) == 0)
 ...
 572                     ; 148 		GPIO_WriteLow(IIC_SDA_PORT,IIC_SDA_PIN);
 ...
 581  018a               L571:
 582                     ; 152 		GPIO_WriteHigh(IIC_SDA_PORT,IIC_SDA_PIN);
 ...
 594                     ; 155 	GPIO_WriteHigh(IIC_SCL_PORT,IIC_SCL_PIN);
 ...
 605                     ; 157 	GPIO_WriteLow(IIC_SCL_PORT,IIC_SCL_PIN);
 ...
 616                     ; 160 	return	value;		
 618  01b4 7b02          	ld	a,(OFST-1,sp)
 621  01b6 5b04          	addw	sp,#4
 622  01b8 87            	retf
 663                     ; 163 u8 SWI2C_TestDevice(u8 addr)
 663                     ; 164 {
 664                     	switch	.text
 ...
 672                     ; 166 	_SWI2C_Start();
 674  01bb 8d0d000d      	callf	L15f__SWI2C_Start
 676                     ; 167 	result = _SWI2C_SendByte(addr);
 ...
 686                     ; 170 	return result;
 688  01cb 7b01          	ld	a,(OFST+0,sp)
 691  01cd 85            	popw	x
 692  01ce 87            	retf
 741                     ; 173 u8 SWI2C_ReadByte(u8 addr, u8 subaddr, u8 * pValue)
 741                     ; 174 {
 742                     	switch	.text
 743  01cf               f_SWI2C_ReadByte:
 745  01cf 89            	pushw	x
 746       00000000      OFST:	set	0
 749                     ; 175 	return SWI2C_ReadBytes(addr, subaddr, 1, pValue);
 ...
 763  01e1 85            	popw	x
 764  01e2 87            	retf
 830                     ; 178 u8 SWI2C_ReadBytes(u8 addr, u8 subaddr, u8 number, u8 * p_data)
 830                     ; 179 {	
 831                     	switch	.text
 ...
 934                     ; 208 	return IIC_OK;
 936  0247 a601          	ld	a,#1
 938  0249 20c4          	jra	L62
 984                     ; 211 u8 SWI2C_WriteByte(u8 addr, u8 subaddr, u8 value)
 984                     ; 212 {	
 985                     	switch	.text
 986  024b               f_SWI2C_WriteByte:
 988  024b 89            	pushw	x
 989       00000000      OFST:	set	0
 992                     ; 213 	return SWI2C_WriteBytes(addr, subaddr, 1, &value);
...
1007  025f 85            	popw	x
1008  0260 87            	retf
1063                     ; 216 u8 SWI2C_Write2Byte(u8 addr, u8 subaddr, u16 data) 
1063                     ; 217 {
1064                     	switch	.text
...
1475                     ; 300 	TIM1_OC1Init(TIM1_OCMODE_PWM2, TIM1_OUTPUTSTATE_ENABLE, TIM1_OUTPUTNSTATE_DISABLE,
1475                     ; 301 	           0, TIM1_OCPOLARITY_LOW, TIM1_OCNPOLARITY_HIGH, TIM1_OCIDLESTATE_SET,
1475                     ; 302 	           TIM1_OCNIDLESTATE_RESET);
...
1490                     ; 303 	TIM1_Cmd(ENABLE);
1492  03d9 a601          	ld	a,#1
1493  03db 8d000000      	callf	f_TIM1_Cmd
1495                     ; 304 	TIM1_CtrlPWMOutputs(ENABLE);
...
1555                     ; 307 void SWI2C_Update(void)
1555                     ; 308 {	
1556                     	switch	.text
1557  03e6               f_SWI2C_Update:
1559  03e6 89            	pushw	x
1560       00000002      OFST:	set	2
1563                     ; 309 	if (Backlight_on_timer == TIMER_EXPIRED)
...
1572                     ; 313 		GPIO_WriteLow(BACKLIGHT_ONOFF_PORT, BACKLIGHT_ONOFF_PIN);	
1574  03f7 4b80          	push	#128
1575  03f9 ae500a        	ldw	x,#20490
1576  03fc 8d000000      	callf	f_GPIO_WriteLow
1578  0400 84            	pop	a
1579                     ; 314 		Backlight_on_timer = TIMER_STOPPED;
...
1585  040d               L544:
1586                     ; 317 	if (Power_status && !I2C_stop)
...
1597                     ; 321 		if (frc_update_timer == TIMER_EXPIRED && Have_FRC)
...
1609                     ; 324 			SWI2C_ReadByte(FRC_BOARD_ADDRESS, 0x18, &read_LVDS_mode);
...
1619  0442 85            	popw	x
1620                     ; 325 			if (read_LVDS_mode != LVDS_mode)
1622  0443 7b01          	ld	a,(OFST-1,sp)
1623  0445 c10008        	cp	a,L7_LVDS_mode
1624  0448 270e          	jreq	L354
1625                     ; 327 				SWI2C_WriteByte(FRC_BOARD_ADDRESS, 0x18, LVDS_mode);
...
1633  0457 84            	pop	a
1634  0458               L354:
1635                     ; 329 			SWI2C_ReadByte(FRC_BOARD_ADDRESS, 0x0A, &read_MFC);
...
2027  000b 00            	dc.b	0
2080                     ; 403 static void SWI2C_Set3DOnOff(u8 OnOff)
2080                     ; 404 {
2081                     	switch	.text
...
2092                     ; 408 		reg_value = 0x80;
2094  05a6 a680          	ld	a,#128
2095  05a8 6b03          	ld	(OFST+0,sp),a
2097  05aa 2002          	jra	L165
2098  05ac               L755:
2099                     ; 412 		reg_value = 0x0;
2101  05ac 0f03          	clr	(OFST+0,sp)
2102  05ae               L165:
2103                     ; 414 	for (retry = 0; retry < 3; retry++)
2105  05ae 0f02          	clr	(OFST-1,sp)
2106  05b0               L365:
2107                     ; 417 		SWI2C_WriteByte(FPGA_ADDRESS, 0x57, reg_value);
...
2116  05bd 84            	pop	a
2117                     ; 418 		SWI2C_ReadByte(FPGA_ADDRESS, 0x57, &value);
...
2148  05de 87            	retf
2173                     ; 426 void SWI2C_Toggle3DOnOff(void)
2173                     ; 427 {	
2174                     	switch	.text
2175  05df               f_SWI2C_Toggle3DOnOff:
2179                     ; 428 	Set3DOn = !Set3DOn;
...
2189                     ; 429 	SWI2C_Set3DOnOff(Set3DOn);
2191  05ed c6000b        	ld	a,L135_Set3DOn
2192  05f0 8da105a1      	callf	L335f_SWI2C_Set3DOnOff
2194                     ; 430 }
2197  05f4 87            	retf
2234                     ; 434 void FPGA_Init(void)
2234                     ; 435  {	
2235                     	switch	.text
2236  05f5               f_FPGA_Init:
2238  05f5 88            	push	a
2239       00000001      OFST:	set	1
2242                     ; 437 	for (i = 0; i < table_size; i++)
2244  05f6 0f01          	clr	(OFST+0,sp)
2246  05f8 202a          	jra	L326
2247  05fa               L716:
2248                     ; 439 		SWI2C_WriteByte(FPGA_ADDRESS, address_table[i], FLASH_ReadByte(0x4001 + i));
2250  05fa 7b01          	ld	a,(OFST+0,sp)
2251  05fc 5f            	clrw	x
2252  05fd 97            	ld	xl,a
2253  05fe 1c4001        	addw	x,#16385
2254  0601 8d000000      	callf	d_itolx
2256  0605 be02          	ldw	x,c_lreg+2
2257  0607 89            	pushw	x
2258  0608 be00          	ldw	x,c_lreg
2259  060a 89            	pushw	x
2260  060b 8d000000      	callf	f_FLASH_ReadByte
2262  060f 5b04          	addw	sp,#4
2263  0611 88            	push	a
2264  0612 7b02          	ld	a,(OFST+1,sp)
2265  0614 5f            	clrw	x
2266  0615 97            	ld	xl,a
2267  0616 d60000        	ld	a,(_address_table,x)
2268  0619 97            	ld	xl,a
2269  061a a6ba          	ld	a,#186
2270  061c 95            	ld	xh,a
2271  061d 8d4b024b      	callf	f_SWI2C_WriteByte
2273  0621 84            	pop	a
2274                     ; 437 	for (i = 0; i < table_size; i++)
2276  0622 0c01          	inc	(OFST+0,sp)
2277  0624               L326:
2280  0624 7b01          	ld	a,(OFST+0,sp)
2281  0626 c10000        	cp	a,_table_size
2282  0629 25cf          	jrult	L716
2283                     ; 441 	if (FLASH_ReadByte(0x4001))
2285  062b ae4001        	ldw	x,#16385
2286  062e 89            	pushw	x
2287  062f ae0000        	ldw	x,#0
2288  0632 89            	pushw	x
2289  0633 8d000000      	callf	f_FLASH_ReadByte
2291  0637 5b04          	addw	sp,#4
2292  0639 4d            	tnz	a
2293  063a 2704          	jreq	L726
2294                     ; 443 		Set3DOn = TRUE;
2296  063c 3501000b      	mov	L135_Set3DOn,#1
2297  0640               L726:
2298                     ; 445 	SWI2C_Set3DOnOff(Set3DOn);	
2300  0640 c6000b        	ld	a,L135_Set3DOn
2301  0643 8da105a1      	callf	L335f_SWI2C_Set3DOnOff
2303                     ; 446 }
2306  0647 84            	pop	a
2307  0648 87            	retf
2340                     ; 448 void HDMI_HotPlug(u8 value)
2340                     ; 449 {
2341                     	switch	.text
2342  0649               f_HDMI_HotPlug:
2346                     ; 450 	if (value)
2348  0649 4d            	tnz	a
2349  064a 270c          	jreq	L546
2350                     ; 452 		GPIO_WriteHigh(HDMI_HOTPLUG_PORT,HDMI_HOTPLUG_PIN);
2352  064c 4b40          	push	#64
2353  064e ae5005        	ldw	x,#20485
2354  0651 8d000000      	callf	f_GPIO_WriteHigh
2356  0655 84            	pop	a
2358  0656 200a          	jra	L746
2359  0658               L546:
2360                     ; 456 		GPIO_WriteLow(HDMI_HOTPLUG_PORT,HDMI_HOTPLUG_PIN);
2362  0658 4b40          	push	#64
2363  065a ae5005        	ldw	x,#20485
2364  065d 8d000000      	callf	f_GPIO_WriteLow
2366  0661 84            	pop	a
2367  0662               L746:
2368                     ; 458 }
2371  0662 87            	retf
2373                     	switch	.const
2374  0004               L156_deep_value:
2375  0004 30            	dc.b	48
2376  0005 80            	dc.b	128
2377  0006 60            	dc.b	96
2378  0007 50            	dc.b	80
2379  0008 70            	dc.b	112
2380  0009 70            	dc.b	112
2381  000a 70            	dc.b	112
2382  000b 60            	dc.b	96
2383  000c 80            	dc.b	128
2384  000d 90            	dc.b	144
2385  000e 50            	dc.b	80
2386  000f 90            	dc.b	144
2387  0010 a0            	dc.b	160
2388  0011 40            	dc.b	64
2389  0012 a0            	dc.b	160
2390  0013 a8            	dc.b	168
2391  0014 30            	dc.b	48
2392  0015 a0            	dc.b	160
2425                     ; 470 void SWI2C_Set_deep(u8 deep)
2425                     ; 471 {
2426                     	switch	.text
2427  0663               f_SWI2C_Set_deep:
2429  0663 88            	push	a
2430       00000000      OFST:	set	0
2433                     ; 472 	if (deep == 0)
2435  0664 4d            	tnz	a
2436  0665 2650          	jrne	L766
2437                     ; 474 		SWI2C_WriteByte(FPGA_ADDRESS, 0x59, FLASH_ReadByte(0x4000 + REG_0x59 + 1));
2439  0667 ae400b        	ldw	x,#16395
2440  066a 89            	pushw	x
2441  066b ae0000        	ldw	x,#0
2442  066e 89            	pushw	x
2443  066f 8d000000      	callf	f_FLASH_ReadByte
2445  0673 5b04          	addw	sp,#4
2446  0675 88            	push	a
2447  0676 ae0059        	ldw	x,#89
2448  0679 a6ba          	ld	a,#186
2449  067b 95            	ld	xh,a
2450  067c 8d4b024b      	callf	f_SWI2C_WriteByte
2452  0680 84            	pop	a
2453                     ; 475 		SWI2C_WriteByte(FPGA_ADDRESS, 0x5C, FLASH_ReadByte(0x4000 + REG_0x5C + 1));
2455  0681 ae400d        	ldw	x,#16397
2456  0684 89            	pushw	x
2457  0685 ae0000        	ldw	x,#0
2458  0688 89            	pushw	x
2459  0689 8d000000      	callf	f_FLASH_ReadByte
2461  068d 5b04          	addw	sp,#4
2462  068f 88            	push	a
2463  0690 ae005c        	ldw	x,#92
2464  0693 a6ba          	ld	a,#186
2465  0695 95            	ld	xh,a
2466  0696 8d4b024b      	callf	f_SWI2C_WriteByte
2468  069a 84            	pop	a
2469                     ; 476 		SWI2C_WriteByte(FPGA_ADDRESS, 0x5A, FLASH_ReadByte(0x4000 + REG_0x5A + 1));
2471  069b ae400c        	ldw	x,#16396
2472  069e 89            	pushw	x
2473  069f ae0000        	ldw	x,#0
2474  06a2 89            	pushw	x
2475  06a3 8d000000      	callf	f_FLASH_ReadByte
2477  06a7 5b04          	addw	sp,#4
2478  06a9 88            	push	a
2479  06aa ae005a        	ldw	x,#90
2480  06ad a6ba          	ld	a,#186
2481  06af 95            	ld	xh,a
2482  06b0 8d4b024b      	callf	f_SWI2C_WriteByte
2484  06b4 84            	pop	a
2486  06b5 203f          	jra	L176
2487  06b7               L766:
2488                     ; 480 		SWI2C_WriteByte(FPGA_ADDRESS, 0x59, deep_value[deep][0]);
2490  06b7 7b01          	ld	a,(OFST+1,sp)
2491  06b9 97            	ld	xl,a
2492  06ba a603          	ld	a,#3
2493  06bc 42            	mul	x,a
2494  06bd d60004        	ld	a,(L156_deep_value,x)
2495  06c0 88            	push	a
2496  06c1 ae0059        	ldw	x,#89
2497  06c4 a6ba          	ld	a,#186
2498  06c6 95            	ld	xh,a
2499  06c7 8d4b024b      	callf	f_SWI2C_WriteByte
2501  06cb 84            	pop	a
2502                     ; 481 		SWI2C_WriteByte(FPGA_ADDRESS, 0x5C, deep_value[deep][1]);
2504  06cc 7b01          	ld	a,(OFST+1,sp)
2505  06ce 97            	ld	xl,a
2506  06cf a603          	ld	a,#3
2507  06d1 42            	mul	x,a
2508  06d2 d60005        	ld	a,(L156_deep_value+1,x)
2509  06d5 88            	push	a
2510  06d6 ae005c        	ldw	x,#92
2511  06d9 a6ba          	ld	a,#186
2512  06db 95            	ld	xh,a
2513  06dc 8d4b024b      	callf	f_SWI2C_WriteByte
2515  06e0 84            	pop	a
2516                     ; 482 		SWI2C_WriteByte(FPGA_ADDRESS, 0x5A, deep_value[deep][2]);
2518  06e1 7b01          	ld	a,(OFST+1,sp)
2519  06e3 97            	ld	xl,a
2520  06e4 a603          	ld	a,#3
2521  06e6 42            	mul	x,a
2522  06e7 d60006        	ld	a,(L156_deep_value+2,x)
2523  06ea 88            	push	a
2524  06eb ae005a        	ldw	x,#90
2525  06ee a6ba          	ld	a,#186
2526  06f0 95            	ld	xh,a
2527  06f1 8d4b024b      	callf	f_SWI2C_WriteByte
2529  06f5 84            	pop	a
2530  06f6               L176:
2531                     ; 484 }
2534  06f6 84            	pop	a
...
2562                     ; 486 void SWI2C_UpdateTimer(void)
2562                     ; 487 {
2563                     	switch	.text
2564  06f8               f_SWI2C_UpdateTimer:
2568                     ; 488 	if (frc_update_timer > TIMER_EXPIRED)
2570  06f8 ae0000        	ldw	x,#L3_frc_update_timer
2571  06fb 8d000000      	callf	d_ltor
2573  06ff ae0016        	ldw	x,#L401
2574  0702 8d000000      	callf	d_lcmp
2576  0706 2509          	jrult	L307
2577                     ; 490 		frc_update_timer--;
2579  0708 ae0000        	ldw	x,#L3_frc_update_timer
2580  070b a601          	ld	a,#1
2581  070d 8d000000      	callf	d_lgsbc
2583  0711               L307:
2584                     ; 492 	if (Backlight_on_timer > TIMER_EXPIRED)
2586  0711 ae0004        	ldw	x,#L5_Backlight_on_timer
2587  0714 8d000000      	callf	d_ltor
2589  0718 ae0016        	ldw	x,#L401
2590  071b 8d000000      	callf	d_lcmp
2592  071f 2509          	jrult	L507
2593                     ; 494 		Backlight_on_timer--;
2595  0721 ae0004        	ldw	x,#L5_Backlight_on_timer
2596  0724 a601          	ld	a,#1
2597  0726 8d000000      	callf	d_lgsbc
2599  072a               L507:
2600                     ; 496 }
2603  072a 87            	retf
2677                     	xref	_table_size
2678                     	xref	_address_table
2679                     	switch	.bss
2680  0000               L51_Have_FRC:
2681  0000 00            	ds.b	1
2682                     	xref	f_IT6802_fsm
2683                     	xref	f_IT6802_fsm_init
2684                     	xdef	f_SWI2C_UpdateTimer
2685                     	xdef	f_SWI2C_Set_deep
2686                     	xdef	f_SWI2C_Toggle3DOnOff
2687                     	xdef	f_HDMI_HotPlug
2688                     	xdef	f_FPGA_Init
2689                     	xdef	f_SWI2C_TestDevice
2690                     	xdef	f_SWI2C_Write2Byte
2691                     	xdef	f_SWI2C_WriteBytes
2692                     	xdef	f_SWI2C_WriteByte
2693                     	xdef	f_SWI2C_ReadBytes
2694                     	xdef	f_SWI2C_ReadByte
2695                     	xdef	f_SWI2C_ToggleI2CMode
2696                     	xdef	f_SWI2C_ProcessPower
2697                     	xdef	f_SWI2C_SystemPowerDown
2698                     	xdef	f_SWI2C_SystemPowerUp
2699                     	xdef	f_SWI2C_Update
2700                     	xdef	f_SWI2C_Init
2701                     	xref	f_IR_DelayNMiliseconds
2702                     	xref	f_FLASH_ReadByte
2703                     	xref	f_TIM1_CtrlPWMOutputs
2704                     	xref	f_TIM1_Cmd
2705                     	xref	f_TIM1_OC1Init
2706                     	xref	f_TIM1_TimeBaseInit
2707                     	xref	f_GPIO_ReadInputPin
2708                     	xref	f_GPIO_WriteLow
2709                     	xref	f_GPIO_WriteHigh
2710                     	xref	f_GPIO_Init
2711                     	xref.b	c_lreg
2731                     	xref	d_lgsbc
2732                     	xref	d_itolx
2733                     	xref	d_lcmp
