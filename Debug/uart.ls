  ...
  19  0001 0008          	dc.w	8
  54                     ; 40 INTERRUPT_HANDLER(UART_ISR, 18)
  54                     ; 41 {
  55                     	switch	.text
  56  0000               f_UART_ISR:
  ...
  63  000b 89            	pushw	x
  66                     ; 42 	uart_rx_buffer[uart_rx_index] = UART1_ReceiveData8();
 ...
 210                     ; 82 }
 213  008e 87            	retf
 262                     ; 84 void UART_Send(u8 * reply, u8 count)
 262                     ; 85 {
 263                     	switch	.text
 ...
 299  00ae 87            	retf
 327                     ; 94 void UART_Init(void)
 327                     ; 95 {
 328                     	switch	.text
 329  00af               f_UART_Init:
 333                     ; 96 	UART1_Init((uint32_t)115200, UART1_WORDLENGTH_8D, UART1_STOPBITS_1, UART1_PARITY_NO, UART1_SYNCMODE_CLOCK_DISABLE, UART1_MODE_TXRX_ENABLE);
 ...
 347                     ; 97 	UART1_ITConfig(UART1_IT_RXNE, ENABLE);
 349  00c7 4b01          	push	#1
 350  00c9 ae0255        	ldw	x,#597
 351  00cc 8d000000      	callf	f_UART1_ITConfig
 353  00d0 84            	pop	a
 354                     ; 98 	UART1_Cmd(ENABLE);
 356  00d1 a601          	ld	a,#1
 357  00d3 8d000000      	callf	f_UART1_Cmd
 359                     ; 100 	UART3_Init((uint32_t)115200, UART3_WORDLENGTH_8D, UART3_STOPBITS_1, UART3_PARITY_NO, UART3_MODE_TXRX_ENABLE);
 ...
 451                     ; 106 void UART_Update(void)
 451                     ; 107 {
 452                     	switch	.text
 453  00f8               f_UART_Update:
 455  00f8 5206          	subw	sp,#6
 456       00000006      OFST:	set	6
 459                     ; 108 	if (uart_received)
 461  00fa 725d0000      	tnz	L7_uart_received
 462  00fe 2604          	jrne	L23
 463  0100 ac6b036b      	jpf	L371
 464  0104               L23:
 465                     ; 111 		uart_received = FALSE;		
 467  0104 725f0000      	clr	L7_uart_received
 468                     ; 114 		memcpy(uart_rxtx_data, uart_rx_buffer, UART_BUFFER_MAX_LENGTH + 2);
 ...
 476                     ; 116 		switch (uart_rxtx_data[DP_COMMAND])
 ...
 492  012b               L63:
 493  012b ac6b036b      	jpf	L371
 494  012f               L731:
 495                     ; 118 			case DATA_COMMAND_WRITE_BYTE:
 495                     ; 119 				checksum = 0x100 - (uart_rxtx_data[DP_DEVICE_ADDR] + uart_rxtx_data[DP_SUB_ADDR] + uart_rxtx_data[DP_SUB_ADDR_2] + uart_rxtx_data[DP_DATA]);
 ...
 504                     ; 120 				if (checksum == uart_rxtx_data[DP_CHECK_SUM])
 506  0140 7b05          	ld	a,(OFST-1,sp)
 507  0142 c10007        	cp	a,L11_uart_rxtx_data+7
 508  0145 2620          	jrne	L102
 509                     ; 122 					ret = SWI2C_WriteBytes(uart_rxtx_data[DP_DEVICE_ADDR], uart_rxtx_data[DP_SUB_ADDR], 1, &uart_rxtx_data[DP_DATA]);
 ...
 522                     ; 123 					if (ret == IIC_FAIL)
 524  015d 0d06          	tnz	(OFST+0,sp)
 525  015f 260a          	jrne	L502
 526                     ; 125 						uart_rxtx_data[DP_COMMAND] = 0xFF;
 528  0161 35ff0002      	mov	L11_uart_rxtx_data+2,#255
 529  0165 2004          	jra	L502
 530  0167               L102:
 531                     ; 130 					uart_rxtx_data[DP_COMMAND] = 0xFE;
 533  0167 35fe0002      	mov	L11_uart_rxtx_data+2,#254
 534  016b               L502:
 535                     ; 132 				UART_Send(uart_rxtx_data, 8);
 537  016b 4b08          	push	#8
 538  016d ae0000        	ldw	x,#L11_uart_rxtx_data
 539  0170 8d8f008f      	callf	f_UART_Send
 541  0174 84            	pop	a
 542                     ; 133 				break;
 544  0175 ac6b036b      	jpf	L371
 545  0179               L141:
 546                     ; 134 			case DATA_COMMAND_READ_BYTE:
 546                     ; 135 				checksum = 0x100 - (uart_rxtx_data[DP_DEVICE_ADDR] + uart_rxtx_data[DP_SUB_ADDR] + uart_rxtx_data[DP_SUB_ADDR_2] + uart_rxtx_data[DP_DATA]);
 ...
 555                     ; 136 				if (checksum == uart_rxtx_data[DP_CHECK_SUM])
 557  018a 7b05          	ld	a,(OFST-1,sp)
 558  018c c10007        	cp	a,L11_uart_rxtx_data+7
 559  018f 2632          	jrne	L702
 560                     ; 138 					ret = SWI2C_ReadBytes(uart_rxtx_data[DP_DEVICE_ADDR], uart_rxtx_data[DP_SUB_ADDR], 1, &uart_rxtx_data[DP_DATA]);
 ...
 573                     ; 139 					uart_rxtx_data[DP_CHECK_SUM] = 0x100 - (uart_rxtx_data[DP_DEVICE_ADDR] + uart_rxtx_data[DP_SUB_ADDR] + uart_rxtx_data[DP_SUB_ADDR_2] + uart_rxtx_data[DP_DATA]);
 ...
 582                     ; 140 					if (ret == IIC_FAIL)
 584  01b9 0d06          	tnz	(OFST+0,sp)
 585  01bb 260a          	jrne	L312
 586                     ; 142 						uart_rxtx_data[DP_COMMAND] = 0xFF;
 588  01bd 35ff0002      	mov	L11_uart_rxtx_data+2,#255
 589  01c1 2004          	jra	L312
 590  01c3               L702:
 591                     ; 147 					uart_rxtx_data[DP_COMMAND] = 0xFE;
 593  01c3 35fe0002      	mov	L11_uart_rxtx_data+2,#254
 594  01c7               L312:
 595                     ; 149 				UART_Send(uart_rxtx_data, 8);
 597  01c7 4b08          	push	#8
 598  01c9 ae0000        	ldw	x,#L11_uart_rxtx_data
 599  01cc 8d8f008f      	callf	f_UART_Send
 601  01d0 84            	pop	a
 602                     ; 150 				break;
 604  01d1 ac6b036b      	jpf	L371
 605  01d5               L341:
 606                     ; 154 					checksum = uart_rxtx_data[DP_DEVICE_ADDR] + uart_rxtx_data[DP_SUB_ADDR] + uart_rxtx_data[DP_SUB_ADDR_2];
 ...
 679                     ; 166 							if (ret == IIC_FAIL)
 681  0242 0d06          	tnz	(OFST+0,sp)
 682  0244 2730          	jreq	L132
 683                     ; 168 								break;
 685                     ; 170 							if (uart_rxtx_data[DP_DEVICE_ADDR] >= 0xA0 && uart_rxtx_data[DP_DEVICE_ADDR] < 0xB0)
 687  0246 c60003        	ld	a,L11_uart_rxtx_data+3
 688  0249 a1a0          	cp	a,#160
 689  024b 250e          	jrult	L532
 691  024d c60003        	ld	a,L11_uart_rxtx_data+3
 692  0250 a1b0          	cp	a,#176
 693  0252 2407          	jruge	L532
 694                     ; 172 								IR_DelayNMiliseconds(20);
 696  0254 ae0014        	ldw	x,#20
 697  0257 8d000000      	callf	f_IR_DelayNMiliseconds
 699  025b               L532:
 700                     ; 163 						for (i = 0; i < 32;i++)
 ...
 716  0274 25af          	jrult	L522
 717  0276               L132:
 718                     ; 200 						uart_rxtx_data[DP_CHECK_SUM] = 0x100 - (uart_rxtx_data[DP_DEVICE_ADDR] + uart_rxtx_data[DP_SUB_ADDR] + uart_rxtx_data[DP_SUB_ADDR_2] + uart_rxtx_data[DP_DATA]);
 ...
 727                     ; 201 						if (ret == IIC_FAIL)
 729  0288 0d06          	tnz	(OFST+0,sp)
 730  028a 260a          	jrne	L142
 731                     ; 203 							uart_rxtx_data[DP_COMMAND] = 0xFF;
 733  028c 35ff0002      	mov	L11_uart_rxtx_data+2,#255
 734  0290 2004          	jra	L142
 735  0292               L322:
 736                     ; 208 						uart_rxtx_data[DP_COMMAND] = 0xFE;
 738  0292 35fe0002      	mov	L11_uart_rxtx_data+2,#254
 739  0296               L142:
 740                     ; 210 					UART_Send(uart_rxtx_data, 8);
 742  0296 4b08          	push	#8
 743  0298 ae0000        	ldw	x,#L11_uart_rxtx_data
 744  029b 8d8f008f      	callf	f_UART_Send
 746  029f 84            	pop	a
 747                     ; 212 				break;
 749  02a0 ac6b036b      	jpf	L371
 750  02a4               L541:
 751                     ; 216 					checksum = 0x100 - (uart_rxtx_data[DP_DEVICE_ADDR] + uart_rxtx_data[DP_SUB_ADDR] + uart_rxtx_data[DP_SUB_ADDR_2] + uart_rxtx_data[DP_DATA]);
 ...
 803                     ; 224 							if (ret == IIC_FAIL)
 805  02f4 0d06          	tnz	(OFST+0,sp)
 806  02f6 271b          	jreq	L152
 807                     ; 226 								break;
 809                     ; 221 						for (i = 0; i < 16;i++)
 ...
 825  0311 25c1          	jrult	L542
 826  0313               L152:
 827                     ; 239 						checksum = uart_rxtx_data[DP_DEVICE_ADDR] + uart_rxtx_data[DP_SUB_ADDR] + uart_rxtx_data[DP_SUB_ADDR_2];
 ...
 862  034a 25dc          	jrult	L552
 863                     ; 244 						uart_rxtx_data[6 + 256] = 0x100 - checksum;
 865  034c a600          	ld	a,#0
 866  034e 1005          	sub	a,(OFST-1,sp)
 867  0350 c70106        	ld	L11_uart_rxtx_data+262,a
 868                     ; 245 						if (ret == IIC_FAIL)
 870  0353 0d06          	tnz	(OFST+0,sp)
 871  0355 260a          	jrne	L562
 872                     ; 247 							uart_rxtx_data[DP_COMMAND] = 0xFF;
 874  0357 35ff0002      	mov	L11_uart_rxtx_data+2,#255
 875  035b 2004          	jra	L562
 876  035d               L342:
 877                     ; 252 						uart_rxtx_data[DP_COMMAND] = 0xFE;
 879  035d 35fe0002      	mov	L11_uart_rxtx_data+2,#254
 880  0361               L562:
 881                     ; 254 					UART_Send(uart_rxtx_data, 256 + 7);
 ...
 894  036b               L371:
 895                     ; 261 }
 898  036b 5b06          	addw	sp,#6
 899  036d 87            	retf
 932                     ; 264 char putchar(char c)
 932                     ; 265 {
 933                     	switch	.text
 934  036e               f_putchar:
 936  036e 88            	push	a
 937       00000000      OFST:	set	0
 940                     ; 267 	UART3_SendData8(c);
 942  036f 8d000000      	callf	f_UART3_SendData8
 945  0373               L503:
 946                     ; 269 	while (UART3_GetFlagStatus(UART3_FLAG_TXE) == RESET);
 948  0373 ae0080        	ldw	x,#128
 949  0376 8d000000      	callf	f_UART3_GetFlagStatus
 951  037a 4d            	tnz	a
 952  037b 27f6          	jreq	L503
 953                     ; 271 	return (c);
 955  037d 7b01          	ld	a,(OFST+1,sp)
 958  037f 5b01          	addw	sp,#1
 959  0381 87            	retf
1014                     	xdef	f_putchar
1015                     	xdef	f_UART_Update
1016                     	xdef	f_UART_Init
1017                     	xdef	f_UART_Send
1018                     	xdef	f_UART_PutChar
1019                     	xdef	f_UART_ISR
1020                     	switch	.bss
1021  0000               L11_uart_rxtx_data:
1022  0000 000000000000  	ds.b	263
1023  0107               L5_uart_rx_index:
1024  0107 00            	ds.b	1
1025  0108               L3_uart_rx_buffer:
1026  0108 000000000000  	ds.b	263
1027                     	xref	f_IR_DelayNMiliseconds
1028                     	xref	f_SWI2C_WriteBytes
1029                     	xref	f_SWI2C_ReadBytes
1030                     	xref	f_UART3_GetFlagStatus
1031                     	xref	f_UART3_SendData8
1032                     	xref	f_UART3_Cmd
1033                     	xref	f_UART3_Init
1034                     	xref	f_UART1_ClearITPendingBit
1035                     	xref	f_UART1_GetFlagStatus
1036                     	xref	f_UART1_SendData8
1037                     	xref	f_UART1_ReceiveData8
1038                     	xref	f_UART1_ITConfig
1039                     	xref	f_UART1_Cmd
1040                     	xref	f_UART1_Init
1041                     	xref.b	c_x
1042                     	xref.b	c_y
1062                     	xref	d_lcmp
1063                     	xref	d_ltor
