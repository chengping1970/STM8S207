...
void UART_Send(u8 * reply, u16 count)
{
...
#if UART_FOR_WALL
	#if DEBUG_USE_UART1
	UART3_Init((uint32_t)9600, UART3_WORDLENGTH_8D, UART3_STOPBITS_1, UART3_PARITY_NO, UART3_MODE_TX_ENABLE);
	#else
	...
	#if DEBUG_USE_UART1
		UART3_Init((uint32_t)9600, UART3_WORDLENGTH_8D, UART3_STOPBITS_1, UART3_PARITY_NO, UART3_MODE_RX_ENABLE);
	#else
